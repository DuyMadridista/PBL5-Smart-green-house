...
function ensureFileURL(value) {
  return isFileURL(value) ? value : node_url.pathToFileURL(value).toString();
}
function ensurePath(value) {
  return isFileURL(value) ? node_url.fileURLToPath(value) : value;
}
...
function ensureTrailingSeparator(dirname2) {
  const separator = isFileURL(dirname2) ? "/" : node_path.sep;
...
function getModuleFilename(dirname2, filename) {
  if (isInESModuleScope()) {
    if (isFileURL(filename)) {
      ...
      return new node_url.URL(filename, ensureFileURL(validatedDirname)).toString();
    }
  } else {
    return node_path.resolve(ensurePath(dirname2), ensurePath(filename));
  }
}
...
function resolveModuleSpecifier(specifier, dirname2) {
  ...
  return specifier.startsWith(".") || node_path.isAbsolute(specifier) ? node_path.resolve(ensurePath(dirname2), specifier) : specifier;
}
...
function requireFromString(code, { filename = `${nanoid()}.js`, dirname = getCallerDirname(), globals = {}, useCurrentGlobal = false } = {}) {
  ...
  const contextModule = new node_module.Module(moduleFilename, mainModule);
  contextModule.require = node_module.createRequire(moduleFilename);
...
async function templateRender(name, code, options, config) {
  ...
  app.use(index.VueEmailPlugin, config?.options);
...
function correctName(name) {
  return scule.pascalCase(name.replace(":", "-").replace(".vue", ""));
}
...
function compile(filename, source, verbose = false) {
  ...
  const { descriptor, errors } = compiler__namespace.parse(source, {
    filename
  });
  if (errors.length) {
  ...
  if (descriptor.script || descriptor.scriptSetup)
    script = compiler__namespace.compileScript(descriptor, {
...
const config = (dir, config2 = {}) => {
  ...
  return {
    render: (name, options) => {
      const path = dir ? node_path.resolve(dir, name) : name;
...
function readFile(path) {
  return node_fs.readFileSync(path, "utf-8").toString();
}
function getAllVueComponents(emailsPath, basePath = "") {
  const result = [];
  const files = node_fs.readdirSync(emailsPath);
  files.forEach((file) => {
    const filePath = node_path.join(emailsPath, file);
    const relativePath = node_path.join(basePath, file);
    if (node_fs.statSync(filePath).isDirectory()) {
      result.push(...getAllVueComponents(filePath, relativePath));
    } else if (node_path.extname(file) === ".vue") {
      result.push({
        name: relativePath.replace(/\\/g, ":"),
        source: node_fs.readFileSync(filePath, "utf8")
