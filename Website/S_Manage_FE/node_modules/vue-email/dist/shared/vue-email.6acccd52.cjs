...
const twToCss = require('@flowko/tw-to-css');
...
const ETailwind = vue.defineComponent({
  ...
  async setup(props, { slots }) {
    ...
    const { twi } = twToCss.tailwindToCSS({ config: props.config });
    const fullHTML = await serverRenderer.renderToString(vue.h("div", $default)).then((html2) => html2.replace(/^<div[^>]*>|<\/div>$/g, ""));
    ...
    return () => {
      return vue.h("template", { innerHTML: html });
    };
  },
  render() {
    return vue.h("render");
...
exports.convertStyleStringToObj = convertStyleStringToObj;
...
exports.xmlDecodeTree = xmlDecodeTree;
